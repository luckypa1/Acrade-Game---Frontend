...
var Engine = (function(global) {
    ...
    function update(dt) {
        ...
        switch (currentGameState) {
            case "startGame":
                ...
                break;
            // Here we do the "normal" things we'd do when the game is running, mainly resentData
            case "inGame":
                // Turn the keypress event listener in app.js back on
                document.addEventListener("keyup", input);
                // Call resentData to update each entity in the game
                resentData(dt);
                // Fix player head staying rendered behind top tiles
                // help from https://discussions.udacity.com/t/canvas-not-clearing-player-bug-fixed/29714
            ...
            case "gameOver":
                ...
                document.removeEventListener('keyup', input);
                // Listen for enter key, switch game state to inGame when pressed
                var gameoverData = function(e) {
                    var key = e.which || e.keyCode;
                    if (key === 13) {
                        currentGameState = "inGame";
                    }
                };
                document.addEventListener("keydown", gameoverData);
                break;
        }
    }
        ...
        case "startGame":
            ...
            for (row = 0; row < numRows; row++) {
                for (col = 0; col < numCols; col++) {
                    ...
                    ctx.fillText("Let's Play The Game!", canvas.width/2, canvas.height/5.5);
                    ctx.fillStyle = "brown";
                    ctx.font = "20px Comic Sans MS";
                    ctx.textAlign = "center";
                    ctx.fillText("Press Enter To Start the Game!!", canvas.width/2, canvas.height/4);
                    ctx.fillStyle = "black";
                    ...
                    ctx.font = "20px Comic Sans MS";
                    ctx.textAlign = "center";
                    ctx.fillText("TRY YOUR LUCK KIDO!!", canvas.width/2, canvas.height/2.1);
                }
            }
            break;
        case "inGame":
            // This array contains all the images assigned in each row.
            rowImages = [
                    'images/water-block.png',   // Top row is water
                    'images/stone-block.png',   // Row 1 of 3 of stone
                    'images/stone-block.png',   // Row 2 of 3 of stone
        ...
        case "gameOver":
            ...
            for (let row = 0; row < numRows; row++) {
                for (let col = 0; col < numCols; col++) {
                    ...
                    ctx.font = "40px Comic Sans MS";
                    ctx.textAlign = "center";
                    ctx.fillText("YOU ARE FINSHED !", canvas.width/2, canvas.height/3);
    ...
    function reset() {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            let startSpeed = speedMultiplier * Math.floor(Math.random() * 10 + 1);
