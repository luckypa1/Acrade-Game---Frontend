...
Enemy.prototype.update = function(dt) {
    ...
    // which will ensure the game runs at the same move for
    // all computers.
    this.x = this.x + this.move * dt;
    // Reset the enemy with a new move after it goes off screen
    this.offScreenX = 505;
    this.startingX = -100;
...
// move Multiplier, we increase this value to increase difficulty
...
Enemy.prototype.randommove = function (){
    "use strict";
    // move is a random number from 1-10 times moveMultiplier
    this.move = moveMultiplier * Math.floor(Math.random() * 10 + 1);
};
...
Enemy.prototype.render = function() {
    ...
    ctx.fillText("Difficulty: " + moveMultiplier, 260, 70);
};
...
for (var i = 0; i < 3; i++) {
    //startmove is a random number from 1-10 times moveMultiplier
    var startmove = moveMultiplier * Math.floor(Math.random() * 10 + 1);
    //enemys start off canvas (x = -100) at the following Y positions: 60, 145, 230
    allEnemies.push(new Enemy(-100, 60 + (85 * i), startmove));
}
